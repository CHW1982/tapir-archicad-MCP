...
-   **Tapir Add-On (REQUIRED)**: The [Tapir Archicad Add-On](https://github.com/ENZYME-APD/tapir-archicad-automation) is **critically required** for this server to function. Without it:
    ...
    **Installation guide:** Follow the [Tapir installation instructions](https://github.com/ENZYME-APD/tapir-archicad-automation#installation) to download and install the Add-On for your Archicad version.
...
-   **Tapir Add-On (REQUIRED):** Extends Archicad's built-in JSON API with 80+ additional commands and enables instance discovery.
-   **Archicad JSON API:** Archicad's official JSON interface for automation.

> **Note:** The Tapir Add-On is a critical dependency. It not only provides additional commands but also enables the server to discover and identify running Archicad instances. The server will not function correctly without it.
